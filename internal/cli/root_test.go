...
func TestRunInitRendersJSONEnvelope(t *testing.T) {
	...
	exitCode := Run([]string{"--json", "init", "--project-key", "PROJ"}, stdout, stderr)
	...
	if env.Counts.Created != 1 {
		t.Fatalf("expected one created result, got %#v", env.Counts)
	}

	if stderr.Len() != 0 {
		t.Fatalf("did not expect diagnostics on stderr: %q", stderr.String())
	}
}
